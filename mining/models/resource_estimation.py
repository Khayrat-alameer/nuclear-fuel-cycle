...
class ResourceEstimationModel:
    ...
    def _kriging_estimation(self) -> Dict[str, np.ndarray]:
        ...
        global_mean = np.mean(drill_grades)
        ...
        for i in range(len(self.grid_points)):
            ...
            if len(neighbor_indices) < self.params.min_neighbors:
                # Use global mean with some variation
                estimated_grades[i] = global_mean * np.random.uniform(0.8, 1.2)
                estimation_variance[i] = np.var(drill_grades)
                continue

            # Keep closest neighbors only
            n_keep = min(len(neighbor_indices), self.params.max_neighbors)
            closest_idx = np.argpartition(dist_row, n_keep)[:n_keep]
            neighbor_grades = drill_grades[closest_idx]
            neighbor_dists = dist_row[closest_idx]

            # Avoid division by zero
            neighbor_dists = np.maximum(neighbor_dists, 1e-6)
        ...
        # Calculate total resource (simplified - assume average of grid points above cutoff)
        grades = estimated_grades[estimated_grades >= self.params.grade_cutoff]
        avg_grade = np.mean(grades) if len(grades) > 0 else 0

        volume = len(estimated_grades) * (self.params.grid_resolution**3)  # m³
        density = 10.0  # g/cm³ for U3O8 → 10000 kg/m³
        total_kg = volume * density * avg_grade

        return {
            "estimated_grades": estimated_grades,
            "estimation_variance": estimation_variance,
            "method": "kriging",
            "total_resource_kg": total_kg,
            "average_grade": avg_grade,
            "recoverable_kg": total_kg * 0.8,
        }
    ...
    def _inverse_distance_estimation(self) -> Dict[str, np.ndarray]:
        ...
        for i, (grid_point, dist_row) in enumerate(zip(self.grid_points, distances)):
            ...
            if len(neighbor_indices) > self.params.max_neighbors:
                closest_indices = np.argsort(dist_row[neighbor_mask])[
    ...
    def calculate_grade_tonnage_curve(
        ...
        cutoff_grades = np.linspace(
            self.params.grade_cutoff, np.max(economic_grades), 20
        )
        tonnages = []
        average_grades = []
        ...
        for cutoff in cutoff_grades:
            ...
            if np.any(mask):
                ...
                avg_grade = np.average(
                    economic_grades[mask], weights=economic_tonnage[mask]
                )
...
def example_resource_estimation():
    ...
    print(
        f"Estimated economic tonnage: {resource_classification['total_economic_tonnage']} grid cells"
    )
